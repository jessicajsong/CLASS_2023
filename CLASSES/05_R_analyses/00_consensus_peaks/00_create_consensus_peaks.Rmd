...
library(tidyverse)
library(IRanges)
source('/scratch/Shares/rinnclass/CLASS_2023/jeso3380/CLASS_2023/util/BCHM5631_JS_functions.R')

# filepath to import peaks
basepath <- "/scratch/Shares/rinnclass/CLASS_2023/kurt/CLASS_2023"
peak_path <- "group_chip/Jessica/results/bwa/mergedLibrary/macs/broadPeak/"
broadpeakfilepath <- file.path(basepath, peak_path)

basepath <- "/scratch/Shares/rinnclass/CLASS_2023/jeso3380/CLASS_2023"
peak_path <- "CLASSES/03_Nextflow/01_my_chipseq/results/bwa/mergedLibrary/macs/broadPeak/"
broadpeakfilepath <- file.path(basepath, peak_path)
```
...
peak_1 <- read_tsv("ATF3_consensus_peaks.bed", col_names = F)
...
```{r reading .bed file from rtracklayer_export}
# FIRST a file list:
consensus_file_list <- list.files("/scratch/Shares/rinnclass/CLASS_2023/jeso3380/CLASS_2023/CLASSES/05_R_analyses/00_consensus_peaks", full.names = T, pattern = ".bed")
...
There is a lot of information here on file formats for UCSC:
...
new_filenames <- paste0("ucsc_consensus_peaks_v2/", names(peaks), ".bed")
new_filenames

### No / before ucsc_consensus_peaks!!!
...
```{R}
# let's do some more indexing and only print out the first two of the three files.
...
(2) go to class UCSC account BCHM_5631 (password is the same)
(3) custom tracks and load consensus file.
